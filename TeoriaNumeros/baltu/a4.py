...
LIMIT = 10**6 # raiz cuadrada de 10^12
primos = Primos(LIMIT)
...
for n in values:
    k = n+1
    if k <= 2: # caso para llegar bien a 2
        k = 2

        if primos.test_primalidad(k):
            sys.stdout.write(f"{k}\n")
            continue

    # si es par y mayor que 2, se salta al siguiente impar
    if k % 2 == 0:
        k += 1
    
    # busqueda del siguiente primo
    while not primos.test_primalidad(k):
        k += 2  # solo impares
